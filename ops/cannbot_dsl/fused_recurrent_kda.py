    ...
    def __call__(
        ...
        if get_block_idx() < logical_core_num:
            for item in range(item_start, item_end):
                ...
                query_length = gm_query_lengths[batch_index]
                if query_length > seq_len:
                    query_length = seq_len
                accepted_tokens = gm_na[batch_index]
                # Padding rows must not read or write the shared state pool.
                # The accepted count selects a previous-step snapshot, so it
                # can exceed this step's query length while still fitting S.
                if query_length > 0 and accepted_tokens > 0 and accepted_tokens <= seq_len:
                    initial_state_index = gm_ssm[base + accepted_tokens - 1]
                    if initial_state_index >= 0 and initial_state_index < gm_initial_state.shape[0]:
                        vector.load_state(tile_view(gm_initial_state[initial_state_index, value_head, None, None],
                                          (row_block, head_dim),(row_index, 0)))
                        for token in range(0, query_length, 1):
                            ...
                            if snapshot_state_index >= 0 and snapshot_state_index < gm_final_state.shape[0]:
...
class FusedRecurrentKDA:
    ...
    def run(self, key: Tensor, query: Tensor, raw_g: Tensor, value: Tensor, raw_beta: Tensor,
        ...
        na: Tensor, query_lengths: Tensor, seq_len: int):
        ...
        op[self.block_num](
            ...
            initial_state, final_state, ssm, na, query_lengths, seq_len,
        )
...
def fused_recurrent_kda_functional(
    ...
    *,
    ...
    ``query_lengths`` is optional int32 ``[B]`` metadata for real query
    lengths inside the fixed-width batch. Zero-length rows leave state alone.
    """
    ...
    tensors = (query, key, value, beta, g, A_log, dt_bias)
    assert all(tensor.device == state.device for tensor in tensors), "all inputs must share one device"
    assert all(tensor.is_contiguous() for tensor in tensors), "sequence and gate inputs must be contiguous"
    if layout_qkv == "BSND":
        # MRV2 places contiguous [H,V,K] states inside padded physical pages.
        # BSND compilation already retains the actual state TensorSpec stride.
        assert tuple(state.stride()[1:]) == (dim * dim, dim, 1), \
            "state must be contiguous within each [H,V,K] page"
        assert state.stride(0) >= num_value_heads * dim * dim, "state pages must not overlap"
    else:
        assert state.is_contiguous(), "BNSD state must be contiguous"
    ...
    if query_lengths is not None:
        assert query_lengths.dtype == torch.int32, \
            f"query_lengths must be int32, got {query_lengths.dtype}"
        assert tuple(query_lengths.shape) == (batch,), f"query_lengths must have shape ({batch},)"
        assert query_lengths.device == device and query_lengths.is_contiguous(), \
            "query_lengths must be contiguous on the input device"
    ...
    if query_lengths is None:
        query_lengths_i64 = torch.full((batch,), seq_len, dtype=torch.int64, device=device)
    else:
        query_lengths_i64 = query_lengths.to(torch.int64)
    assert ssm_i64.is_contiguous() and na_i64.is_contiguous() and query_lengths_i64.is_contiguous()
    ...
    if layout_qkv == "BSND":
        static_layout = tuple(
            ...
            for tensor in (
                ...
                initial_state, final_state, ssm_i64, na_i64, query_lengths_i64,
            )
        )
    cache_key = (
    ...
    if fn is None:
        ...
        if layout_qkv == "BSND":
            compile_args = (
                ...
                _static_tensor_spec(dtypes.int64, query_lengths_i64),
            )
        else:
            compile_args = (
    ...
    fn(
        ...
        na_i64, query_lengths_i64, seq_len,
    )
    return state, out
...
_GRAPH_LIBRARY.define(
    ...
    "Tensor? ssm_state_indices=None, Tensor? num_accepted_tokens=None, Tensor? query_lengths=None) -> Tensor"
)
_GRAPH_LIBRARY.define(
    "fused_recurrent_kda_functional("
    "Tensor query, Tensor key, Tensor value, Tensor state, Tensor beta, Tensor g, "
    "float scale, Tensor A_log, Tensor dt_bias, float lower_bound, str layout_qkv, "
    "Tensor? ssm_state_indices=None, Tensor? num_accepted_tokens=None, Tensor? query_lengths=None) -> (Tensor, Tensor)"
)
...
def _fused_recurrent_kda_meta(
    ...
    del lower_bound, ssm_state_indices, num_accepted_tokens, query_lengths, layout_qkv
